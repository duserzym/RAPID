...
Private Sub ButtonYSet_Click()
    ...
    MoveMotorXY MotorChangerY, val(LTrim$(txtNewY.text)), ChangerSpeed, False, True
End Sub
...
Public Sub ChangerMotortoHole(ByVal hole As Double, Optional ByVal waitingForStop As Boolean = True, Optional ByVal pauseOverride As Boolean = True)
    ...
    Else
        ...
        'Experimental:
        'First Move Towards the corner
        ''MoveMotorAbsoluteXY MotorChanger, (targetX + 100000), ChangerSpeed, False, pauseOverride
        'MoveMotorAbsoluteXY MotorChangerY, (targetY + 100000), ChangerSpeed, waitingForStop, pauseOverride
        'DelayTime 2
...
Public Function ConvertPosToHoleXY(posX As Long, posY As Long) As Double
   ...
   For i = 1 To 101
        ...
        TestXb = TestX < 1000
        TestY = posY - modConfig.XYTablePositions(i, 1)
        TestYb = TestY < 1000
...
Public Function HomeToCenter(ByRef xPos As Long, ByRef yPos As Long, Optional pauseOveride As Boolean = False)
    ...
    Dim StopX As Boolean
    Dim StopY As Boolean
    ...
    If CheckInternalStatus(MotorUpDown, 4) = 0 Then
         SetCodeLevel CodeRed
    ...
    ' if both switches already tripped, do nothing
    'If ((CheckInternalStatus(MotorChanger, 5) = 0) And (CheckInternalStatus(MotorChangerY, 6) = 0)) Then Exit Function
   
    'Do While ((MotorsLocked <> 0) And (MotorsLocked <> MotorChanger))
    '    DelayTime 0.05
    'Loop
   
    'Do While ((MotorsLocked <> 0) And (MotorsLocked <> MotorChangerY))
    '    DelayTime 0.05
    'Loop
    
    'Lock the Changer Motors
    'LockMotor MotorChanger
    'LockMotor MotorChangerY
   
   
    'T Shuma - Move to negative edge
    Do While ((CheckInternalStatus(MotorChanger, 4) <> 0) Or _
              (CheckInternalStatus(MotorChangerY, 5) <> 0)) And Not NOCOMM_MODE And Not Prog_halted
    
        DelayTime 0.05
        
        If ((CheckInternalStatus(MotorChanger, 4) <> 0) And Not StopX) Then
            
            LockMotor MotorChanger
            MoveMotorXY MotorChanger, -30000000, ChangerSpeed, False, pauseOveride, -1, 0
            StopX = True
        
        End If
        
        If ((CheckInternalStatus(MotorChangerY, 5) <> 0) And Not StopY) Then
            
            LockMotor MotorChangerY
            MoveMotorXY MotorChangerY, -30000000, ChangerSpeed, False, pauseOveride, -2, 0
            StopY = True
    ...
    'T Shuma - Now Move to positive limit switches
    If ((CheckInternalStatus(MotorChanger, 4) = 0) And (CheckInternalStatus(MotorChangerY, 5) = 0)) Then
        
        Do While (CheckInternalStatus(MotorChanger, 5) <> 0) Or (CheckInternalStatus(MotorChangerY, 6) <> 0) And Not NOCOMM_MODE
            
            DelayTime 0.05
            
            If CheckInternalStatus(MotorChanger, 5) <> 0 Then
                LockMotor MotorChanger
                MoveMotorXY MotorChanger, 30000000, ChangerSpeed, False, pauseOveride, -2, 0
                StopX = True
            End If
            
            If CheckInternalStatus(MotorChangerY, 6) <> 0 Then
                LockMotor MotorChangerY
                MoveMotorXY MotorChangerY, 30000000, ChangerSpeed, False, pauseOveride, -3, 0
                StopY = True
            End If
    ...
    Else
        ...
        err_msg = "Homed XY stage to center but did not hit negative limit switch(es)!"
        
        
        SetCodeLevel CodeRed
        frmSendMail.MailNotification "XY Stage Homing error!", err_msg, CodeRed
    ...
    'If Not (CheckInternalStatus(MotorChanger, 4) = 0 And CheckInternalStatus(MotorChangerY, 4) = 0) Then
    ...
    If Not (CheckInternalStatus(MotorChanger, 5) = 0 And CheckInternalStatus(MotorChangerY, 6) = 0) And Not NOCOMM_MODE Then
        ...
        SetCodeLevel CodeRed
        frmSendMail.MailNotification "Homing error!", "Homed XY Stage to center but did not hit positive limit switch(es).", CodeRed
...
Public Function HomeToTop(Optional pauseOveride As Boolean = False) As Long
    ...
    ' if switch already tripped, do nothing
    If CheckInternalStatus(MotorUpDown, 4) = 1 Then
    ...
    '(November, 2010 - I Hilburn) Added two additional arguments
    'that tell the move-motor to stop movement once IOinput #1 is set to true
    MoveMotor MotorUpDown, -2 * MeasPos, speed, True, pauseOveride, -1, 1
   
    'Check to see if the motor has reached the limit switch
    If Not CheckInternalStatus(MotorUpDown, 4) = 1 Then
...
Public Sub MoveMotor(motorid As Integer, _
    ...
    ElseIf UseXYTableAPS And motorid = MotorTurning Then
    
        Dim turning_acceleration As Long
        turning_acceleration = 4831
        
        Dim turning_velocity As Long
        turning_velocity = MoveMotorVelocityEdit
        
        If (turning_velocity > 157950000) Then
        
            turning_acceleration = turning_velocity / 157950000 * turning_acceleration * 1.1
        ...
        SendCommand motorid, _
               (MotorAddress(motorid) & "134 " + _
                MoveMotorPosEdit + " " + str$(turning_acceleration) + " " + _
                MoveMotorVelocityEdit + " " + _
                Trim(str(StopEnable)) + " " + _
                Trim(str(StopCondition)))
...
Public Sub MoveMotorAbsoluteXY(motorid As Integer, _
                      MoveMotorPos As Long, _
                      MoveMotorVelocity As Long, _
                      Optional waitingForStop As Boolean = True, _
                      Optional ByVal pauseOverride As Boolean = False, _
...
Public Function MoveToCorner(Optional pauseOveride As Boolean = False)
    ...
    If CheckInternalStatus(MotorUpDown, 4) = 0 Then
         SetCodeLevel CodeRed
        frmSendMail.MailNotification "Homing error!", "Tried to move to corner but UP/Down Motor not homed to top.  This could break control rod.", CodeRed
    ...
    'T Shuma - Move to negative edge
    Do While ((CheckInternalStatus(MotorChanger, 4) <> 0) Or (CheckInternalStatus(MotorChangerY, 5) <> 0)) And Not NOCOMM_MODE
    DelayTime 0.05
    If ((CheckInternalStatus(MotorChanger, 4) <> 0) And Not StopX) Then
    LockMotor MotorChanger
    MoveMotorXY MotorChanger, -3000000, ChangerSpeed, False, pauseOveride, -1, 0
    StopX = True
    End If
    If ((CheckInternalStatus(MotorChangerY, 5) <> 0) And Not StopY) Then
    LockMotor MotorChangerY
    MoveMotorXY MotorChangerY, -3000000, ChangerSpeed, False, pauseOveride, -2, 0
    StopY = True
    End If
    Loop
...
Public Sub TurningMotorRotate(ByVal angle As Double, Optional ByVal waitingForStop As Boolean = True, Optional ByVal pauseOverride As Boolean = True)
    ...
    If Not NOCOMM_MODE And (Abs((CurAngle - angle) Mod 360) > 5) Then ' (November 2009 L Carporzen) Changed 3 to 1 degree
        curpos = ReadPosition(MotorTurning)
...
Public Sub TurningMotorSpin(ByVal speedRPS As Double, Optional ByVal Duration As Double = 60, Optional ByVal pauseOverride As Boolean = False)
    ...
    If Prog_halted Or NOCOMM_MODE Then Exit Sub
    
'    modStatusCode.SetCodeLevel CodeYellow
'
'    ErrorMessage = _
'           "The Berkeley Turning system should not be spun at high-speeds.  " & _
'           "The Spin Motor command has been disabled.  " & _
'           "RRM rockmag steps cannot current be run on this system."
'
'    frmSendMail.MailNotification "Spin Motor Not Allowed.", _
'                                 ErrorMessage & vbNewLine & vbNewLine & _
'                                 "Please come and check the machine.", _
'                                 CodeYellow
'
'    MsgBox ErrorMessage, vbCritical, "Spin Motor Not Allowed."
'
